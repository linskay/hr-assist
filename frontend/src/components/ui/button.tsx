...
const buttonVariants = cva(
  "relative inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-bold uppercase tracking-wider ring-offset-background transition-all duration-300 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        "neon-icon": "text-white bg-black/20 border-2 border-brand-accent/50 hover:border-brand-accent hover:shadow-neon-glow-accent",
      },
      size: {
  ...
  ({ className, variant, size, asChild = false, isLoading = false, children, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        {...props}
      >
        {isLoading ? <LoaderCircle className="mr-2 h-5 w-5 animate-spin" /> : children}
      </Comp>
    )
  }
)
